...
import { 
  ...
  Box, Card, Badge, ActionIcon, Divider 
} from '@mantine/core';
...
function SalesPage() {
  ...
  const handleCreateSale = async () => {
    ...
    try {
      ...
      notifications.show({ title: 'Success', message: 'Sale created' });
      close();
      resetForm();
      loadData();
    } catch (error) {
      notifications.show({
        title: 'Error',
        message: error.response?.data?.ErrorMessage || 'Failed to create sale',
        color: 'red'
      });
    } finally {
      setLoading(false);
  ...
  const rows = sales.slice(0, 20).map((sale) => (
    <Table.Tr key={sale.Id}>
      ...
      <Table.Td>
        <Badge color="green">
          ${sale.Items.reduce((sum, item) => sum + (item.Quantity * item.UnitPrice), 0).toFixed(2)}
        </Badge>
      </Table.Td>
    </Table.Tr>
  ));
  ...
  return (
    <Box>
      ...
      <Card shadow="sm" padding="lg" radius="md">
        <Text fw={500} mb="md">Recent Sales</Text>
      ...
      <Modal opened={opened} onClose={close} title="Create Sale" size="xl">
        <TextInput
          label="Customer Name"
          required
        ...
        <Text fw={500} mb="sm">Add Items</Text>
        <Group grow mb="sm">
          <Select
            label="Species"
        ...
        <Button variant="light" onClick={handleAddItem} mb="md">
          Add Item
        </Button>
        ...
        {saleData.Items.length > 0 && (
          <>
            <Divider mb="md" />
            <Text fw={500} mb="sm">Items:</Text>
            {saleData.Items.map((item, index) => (
              <Group key={index} mb="xs" justify="space-between">
                <Text size="sm">
                  {catalogMap[item.SpeciesId]} - Qty: {item.Quantity} × ${item.UnitPrice.toFixed(2)}
                </Text>
                <Group gap="xs">
